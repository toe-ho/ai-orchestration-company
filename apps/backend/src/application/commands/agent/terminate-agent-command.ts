...
@CommandHandler(TerminateAgentCommand)
export class TerminateAgentHandler implements ICommandHandler<TerminateAgentCommand, IAgent> {
  constructor(
    @Inject(AGENT_REPOSITORY) private readonly agentRepo: IAgentRepository,
  ) {}

  async execute(cmd: TerminateAgentCommand): Promise<IAgent> {
    ...
    const updated = await this.agentRepo.update(cmd.id, { status: 'terminated' });
    return updated!;
  }
}
